#### 부성순 bakpak@empas.com / 010-5155-2953  
#### 구글 공유폴더 :  zip파일 위치   https://drive.google.com/drive/folders/1zdbT8ELimUixLKOXOhBnrhT1sZCm5E9g?usp=drive_link
#### 강의요청사항: https://forms.gle/37KT54JSPZU5kyHu7
#### 딥러닝이해도:  https://forms.gle/FgKtSoyuWDSGVXVe9
#### YOLO이해도: https://forms.gle/jDHkwJzbensuHp9E7 

##### 4번에러있었던 자료는  8번자료와 동일한 작업입니다.  
= 4번의 손모양으로 인식하는 알파벳은 별도 교안으로 제공하겠습니다. 
캐치업시간1: 21일(금) 오전 9시~10시 실습에 손모양 알파벳으로 어노테이션을 제작 & 욜로모델 제작합니다.
캐치업시간2: 21일(금) 오후 3시~4시 에는 부적절한 손모양이 나오면 모자이크 처리를 추가합니다. 


## Part2. 객체인식 
#### 이론: 11.19(수) 13:00 - 11.20(목) 18:00 까지
#### 실기: 11.20(목) 18:00 - 11.24(월) 18:00 까지


#### 11.20일(목)
##### -11:00 : [강의] 어노테이션 & 욜로모델 튜닝
##### 12:00 - 14:00 : [정리 & 실습]
##### 14:00 - 18:00 : SSD모델등 (미션스프린트가 SSD모델입니다)
에스케이-proj-wEq5LHXbAEi0XC6AuwlS_mXcr8W5X2z0ffIl3ilkeCUNc2tZ4nybYSKxMvon8ybffQFsr-dvoST3BlbkFJJ-fv_XKBoJr7pu1G5il_-EGZ6xzHK69iUopLwngdBjpoHHWKRyHW01CdJXwZaB9LXQ1c5Ch84A


## Part3. Image Segmentation
#### 이론: 11.24(월) 18:00 - 11.26(수) 14:00 까지
#### 실기: 11.26(수) 14:00 - 11.28(금) 14:00 까지

## Part4. 이미지 생성
#### 이론: 11.28(금) 14:00 - 12.01(월) 14:00 까지
#### 실기: 12.01(월) 14:00 - 12.02(화) 17:00 까지

## Part5. 파이썬 환경 (for Mac/Windows) & git
#### 이론: 12.02(화) 17:00 - 12.03(수) 12까지   & 12.03(수) 13:00 - 12:05(금) 11:00 까지

## Part6. AI 엔지니어 초급 프로젝트 (12.05(금) 12:00 - 12.23(화) 17:00 )
